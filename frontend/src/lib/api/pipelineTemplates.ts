...
export async function listTemplates(): Promise<PipelineTemplate[]> {
  return jsonOrThrow(await apiFetch(`${API_V1_BASE_URL}/pipeline-templates/`));
}
...
export async function createTemplate(payload: {
  ...
  return jsonOrThrow(
    await apiFetch(`${API_V1_BASE_URL}/pipeline-templates/`, {
      method: 'POST',
...
export async function deleteTemplate(id: number): Promise<void> {
  const res = await apiFetch(`${API_V1_BASE_URL}/pipeline-templates/${id}`, { method: 'DELETE' });
  if (!res.ok) {
    const text = await res.text().catch(() => '');
