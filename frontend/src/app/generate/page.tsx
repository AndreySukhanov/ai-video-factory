...
export default function GenerateEpisodePage() {
    ...
    const handleStoryGenerate = async () => {
        ...
        // - Veo 3.1 R2V: uses reference_images array for character consistency
        const subjectReference = (isConsistencyEnabled && storyModel === 'minimax') ? effectiveCharacterImage : null;
        const referenceImagesArray = (isConsistencyEnabled && storyModel === 'veo31' && effectiveCharacterImage) ? [effectiveCharacterImage] : null;

        console.log(`[Story Mode ${storyModel}] Starting generation with consistency=${isConsistencyEnabled}`);
        console.log(`[Story Mode ${storyModel}] Reference: uploaded=${referenceImageUrl ? 'yes' : 'no'}, autoGenerated=${characterImageUrl ? 'yes' : 'no'}, effective=${effectiveCharacterImage ? 'yes' : 'no'}`);
